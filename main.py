...
def main():
    ...
    def open_graph_window():
        ...
        button_draw_graph = tk.Button(output_frame_graph, text="有向图",
                                      command=lambda: show_graph(graph,
                                    output_graph_file, output_text_graph))
        ...
        def show_shortest_path2(graph, word1, word2):
            ...
            for i in enumerate(range(len(shortest_path))):
                output_text2_short.insert(tk.END,
                    f"The shortest path from {word1} to {word2} is: "
                    f"{', '.join(shortest_path[i])}，len = {len(shortest_path[i])}\n")
            ...
            if word1 == '':
                ...
                for i in enumerate(range(len(shortest_path))):
                    output_text_short.insert(tk.END,
                                    f"The shortest path from {word1} to "
                                    f"{word2} is: {', '.join(shortest_path[i])}，len = "
                                    f"{len(shortest_path[i])}\n")
    ...
    def open_gentext_window():
        ...
        button_gen_new_text = tk.Button(output_frame_gentext, text="生成新文本",
                                        command=lambda:gen_new_text(graph))
    ...
    def open_bridge_window():
            ...
            if graph.has_node(word1) and graph.has_node(word2):
                ...
                if bridge_words:
                    ...
                    output_text_bridge.insert(tk.END,
                            f"The bridge words from "
                            f"{word1} to {word2} are: {', '.join(bridge_words)}\n")
                else:
                    output_text_bridge.delete("1.0", tk.END)  # 清空原有内容
                    output_text_bridge.insert(tk.END, f"No bridge "
                                        f"words from {word1} to {word2}!\n")
            else:
                output_text_bridge.delete("1.0", tk.END)  # 清空原有内容
                output_text_bridge.insert(tk.END, f"No {word1} or "
                                f"{word2} in the graph!\n")
        ...
        button_bridge_words = tk.Button(output_frame_bridge,
                                text="桥接词", command=find_bridge_words_button_click)
    ...
    def open_randwalk_window():
        ...
        # 相关的 UI 组件
        button_randwalk = tk.Button(output_frame_randwalk,
                        text="开始随机游走", command=lambda: random_walk(graph))
